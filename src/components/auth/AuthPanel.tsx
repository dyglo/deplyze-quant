...
export interface AuthPanelProps {
  /** Called after a successful sign-in / sign-up / guest upgrade. The full-page
   *  Login leaves this undefined (App re-renders once the user resolves); the
   *  AuthModal uses it to close itself and keep the user on the current page. */
  onSuccess?: () => void;
  headline?: string;
  subcopy?: string;
...
/**
 * AuthPanel — the shared sign-in / sign-up form.
 *
 * Single source of truth for the auth form used by both the full-page Login and
 * the in-context AuthModal. The underlying `useAuth` actions already upgrade an
 * anonymous guest in place (account linking), so signing in here preserves the
 * guest session's uid.
 */
export const AuthPanel: React.FC<AuthPanelProps> = ({ onSuccess, headline, subcopy, defaultMode = 'login' }) => {
  ...
  const handleGoogle = async () => {
    setBusy(true);
    try { await signInWithGoogle(); onSuccess?.(); } catch { /* surfaced via authError */ }
  ...
  return (
    <div style={{ width: '100%' }}>
      <h2 className="ds-title" style={{ margin: 0 }}>
        {headline || (mode === 'login' ? 'Sign in to Deplyze Quant' : 'Create your research desk')}
      </h2>
      <p className="ds-caption" style={{ marginTop: 6, color: 'var(--muted-foreground)' }}>
        {subcopy || (mode === 'login' ? 'Continue to your research workspace.' : 'A workspace will be provisioned for you.')}
      </p>
