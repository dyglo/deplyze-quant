import React from 'react';
import { ShieldAlert } from 'lucide-react';

export const RiskRegimeFit: React.FC = () => (
  <div style={{ padding: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100%' }}>
    <div style={{ textAlign: 'center', maxWidth: '28rem' }}>
      <div style={{
        width: '3rem', height: '3rem', borderRadius: '0.75rem',
        background: 'color-mix(in srgb, var(--destructive) 12%, transparent)',
        border: '1px solid color-mix(in srgb, var(--destructive) 25%, transparent)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        margin: '0 auto 1.25rem',
      }}>
        <ShieldAlert size={20} style={{ color: 'var(--destructive)' }} />
      </div>
      <p style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--foreground)', marginBottom: '0.5rem', letterSpacing: '-0.02em' }}>
        Risk & Regime Fit
      </p>
      <p style={{ fontSize: '0.8125rem', color: 'var(--muted-foreground)', lineHeight: 1.6 }}>
        Portfolio surveillance infrastructure — rolling volatility, rolling beta, drawdown stress, correlation instability maps, risk regime heatmaps, and regime interpretation. Coming in Wave F.
      </p>
    </div>
  </div>
);
